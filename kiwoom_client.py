...
class KiwoomClient:
    ...
    # ══════════════════════════════════════════════════════════
    # ⑪ 미체결 주문 조회  kt00009 → qry_tp:"2" (미체결만)
    #    재시작 시 pending 상태 복구용
    # ══════════════════════════════════════════════════════════

    def get_pending_orders(self) -> list:
        """당일 미체결(또는 부분체결) 주문 목록 조회.
        반환: [{"order_no", "ticker", "action", "ord_qty", "cntr_qty",
                "remaining_qty", "is_partial"}, ...]
        재시작 후 _pending_buy/_pending_sell 복원에 사용.
        """
        payload = {
        ...
        data = self._post("/api/dostk/acnt", "kt00009", payload)
        if not data or data.get("return_code") != 0:
            log.warning(f"[키움] 미체결 조회 실패: {data.get('return_msg','') if data else '응답없음'}")
            return []
        ...
        for o in orders:
            try:
                ord_no    = str(o.get("ord_no", "")).strip()
                ticker    = str(o.get("stk_cd", "")).strip().lstrip("A")
                # sell_tp: "1"=매도, "2"=매수 (키움 코드)
                sell_tp   = str(o.get("sell_tp", "")).strip()
                action    = "sell" if sell_tp == "1" else "buy"
                ord_qty   = int(str(o.get("ord_qty",  "0")).replace(",", "") or "0")
                cntr_qty  = int(str(o.get("cntr_qty", "0")).replace(",", "") or "0")
                remaining = max(0, ord_qty - cntr_qty)
                if not ord_no or not ticker or remaining <= 0:
                    continue  # 이미 전량 체결된 주문은 제외
                result.append({
                    "order_no":     ord_no,
                    "ticker":       ticker,
                    "action":       action,
                    ...
                    "is_partial":   cntr_qty > 0,  # 부분체결 여부
                })
            except Exception as ex:
                log.warning(f"[키움] 미체결 파싱 오류: {ex}")
        log.info(f"[키움] 미체결 주문 {len(result)}건 조회")
        return result
