...
import requests
import schedule
from rt_runtime_guard import append_runtime_audit, save_runtime_snapshot, build_runtime_health_report
from strategy_shared import get_regime_threshold as shared_get_regime_threshold
from strategy_shared import should_enter_by_edge, should_alert_sell_edge, should_reset_sell_edge_alert
...
def get_regime_threshold(regime: str) -> float:
    # 공통 전략 정책으로 위임 (원본 숫자 동일)
    return shared_get_regime_threshold(regime)



def should_enter_regime_edge(edge: float, regime: str, *, slip_ok: bool = True, extra_gate: bool = True) -> bool:
    return should_enter_by_edge(edge, regime, slip_ok=slip_ok, extra_gate=extra_gate).allowed
...
class TelegramCommander:
    ...
    def _recommend(self):
        ...
        for ticker, name in self.monitor.universe.items():
            ...
            if should_enter_regime_edge(edge, regime, slip_ok=slip_ok):
    ...
    def _emergency_stop(self):
        ...
        global EMERGENCY_STOP
        EMERGENCY_STOP = True
        append_runtime_audit("emergency_stop", source="telegram")
        save_runtime_snapshot(monitor=self.monitor, mode=("mock" if kiwoom() and getattr(kiwoom(), "_mock", True) else "live"), emergency_stop=EMERGENCY_STOP)
        tg_btn(
            "🔴 <b>비상정지 완료!</b>\n"
            "━━━━━━━━━━━━━━━━━━\n"
...
class EdgeMonitor:
        ...
        with _positions_lock:
         for ticker, info in list(self.positions.items()):
            ...
            if df is not None:
                ...
                if should_alert_sell_edge(edge_now, sell_thr, info.get("sell_edge_alerted")):
                ...
                elif should_reset_sell_edge_alert(edge_now, sell_thr):
    ...
    def scan_universe(self, force_notify: bool = False):
        ...
        for ticker, name in self.universe.items():
            ...
            if should_enter_regime_edge(edge, self.regime, slip_ok=slip_ok, extra_gate=(edge >= thr)):
...
if __name__ == "__main__":
    ...
    try:
        ...
        _health = build_runtime_health_report(monitor=monitor, mode=_mode, emergency_stop=EMERGENCY_STOP)
        log.info(f"🩺 런타임 상태: {_health}")
        append_runtime_audit("startup", mode=_mode, health=_health)
    except Exception as _e_guard:
        log.warning(f"runtime guard startup skip: {_e_guard}")
    # ── 데이터 소스 진단 ─────────────────────────
    ...
    try:
        ...
        try:
            ...
            save_runtime_snapshot(monitor=locals().get("monitor"), mode=("mock" if (_kw_start and getattr(_kw_start, "_mock", True)) else "live"), emergency_stop=EMERGENCY_STOP)
            append_runtime_audit("shutdown", mode=("mock" if (_kw_start and getattr(_kw_start, "_mock", True)) else "live"), emergency_stop=EMERGENCY_STOP)
